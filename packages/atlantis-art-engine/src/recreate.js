...
const metadata = require("../celestial/json/_metadata.json");
...
const recreateNft = async (metadata, layersOrder) => {
  ...
  return new Promise((resolve, reject) => {
    const out = fs.createWriteStream(`./output/${metadata.edition}.png`);
...
async function main() {
  ...
  for (let i = 0; i < affected.length; i++) {
    ...
    let metadata = require(`../celestial/json/${edition}.json`);
