  ...
  <dialog class="modal" class:modal-open={modalOpen}>
    <div class="modal-box max-w-sm">
      <form method="dialog" class="flex flex-col w-full items-center gap-4">
        ...
        <div class="title-subsection-bold">Supply {market.name}</div>
        ...
        <label class="form-control w-full max-w-xs">
          <div class="label">
            <span class="label-text">Wallet Balance:</span>
        ...
        <button class="btn btn-primary rounded-lg body-semibold text-white bg-pink-500 py-2 px-8">Supply</button>
      </form>
    </div>
  </dialog>
{/if}
